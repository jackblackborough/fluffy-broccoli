...
# New Feature
const DRIFT_BOOST_FORCE = 220
var can_drift_boost = true
...
func _physics_process(delta: float) -> void:
	...
	if not is_on_floor():
		...
		jump_count = 0
		can_drift_boost = true  # Reset drift boost on landing
	...
	# --- NEW FEATURE: AERIAL DRIFT BOOST ---
	if Input.is_action_just_pressed("drift_boost") and not is_on_floor() and can_drift_boost and not is_dashing:
		...
		if input_dir == Vector2.ZERO:
			input_dir = Vector2(0, -1 * gravity_direction)

		input_dir = input_dir.normalized()
		velocity = input_dir * DRIFT_BOOST_FORCE
		can_drift_boost = false
		$AnimatedSprite2D.play("drift_boost")
		$boostSFX.play()
