...
func _physics_process(delta: float) -> void:
	...
	if Input.is_action_just_pressed("jump"):
		...
		if is_on_floor() or infinite_mode or jump_count < max_jumps:
	...
	if not is_on_floor() and velocity.y * gravity_direction < 0 and not is_wall_clinging:
		$AnimatedSprite2D.play("jump")
